...
const appId = "10902c91b9b147ecf81efb5dbddf1ba7"
const link = "http://api.openweathermap.org/data/2.5/air_pollution"	// API end point
...
const getAirQuality = async (lat, lon) => {
	...
	const rawData = await fetch(`${link}?lat=${lat}&lon=${lon}&appid=${appId}`).catch(err => {
		onPosit
		ionGatherError({ message: "Something went wrong. Check your internet conection." })
		console.log(err)
	})
